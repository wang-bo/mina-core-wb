 ...
 * 
 * 1. 一个传播I/O事件到Executor，并强制使用某一线程模型，可以同时处理各个session的事件。
 * 
 * 2. 生命周期：
 * 	a. 这个过滤器并不管理Executor的生命周期，如果你使用Executor做参数的构造方法，
 * 		你有责任来管理Executor的生命周期(如：调用ExecutorService.shutdown())。
 * 	b. 如果你使用了ExecutorFilter(int)这样的构造方法，你可以通过调用destroy()方法来关闭Executor。
 * 
 * 3. 事件触发的顺序：
 * 	a. 使用ExecutorFilter(int)这样的构造方法生成的是OrderedThreadPoolExecutor的实例，此时的事件顺序是：
 * 		1> handler的所有方法同时只能被调用1个(如：messageReceived和messageSent不能被同时调用)。
 * 		2> handler中事件的顺序永远是清晰的(如：messageReceived总是比sessionClosed或messageSent先执行)。
 * 	b. 如果使用了其它的Executor实例，事件顺序就不一定了，可能1个handler的多个方法会被同时调用，且顺序不一定：
 * 		1> handler的所有方法可能被同时调用(如：messageReceived和messageSent被同时调用)。
 * 		2> handler中事件的顺序是混乱的(如：messageReceived可能比sessionClosed或messageSent晚执行)。
 * 	c. 所以需要正确的事件顺序，请使用OrderedThreadPoolExecutor。
 * 
 * 4. 选择性过滤：
 * 	a. 默认情况下，除了sessionCreated、filterWrite、filterClose、filterSetTrafficMask的事件会被提交到底层的Executor执行。
 * 	b. 如果要提交指定事件，可以在构造方法中指定。
 * 
 * 5. 防止OutOfMemoryError：
 * 	a. 构造方法中使用参数IoEventQueueThrottle
 * 
 * @see OrderedThreadPoolExecutor
 * @see UnorderedThreadPoolExecutor
 * @org.apache.xbean.XBean
 * 
 * @date	2016年6月27日 下午2:54:44	completed
 */
public class ExecutorFilter extends IoFilterAdapter {

	/** The list of handled events. 需处理的事件列表 */
    private EnumSet<IoEventType> eventTypes;
    
    /** The associated executor. 本过滤器使用的线程执行器 */
    private Executor executor;
    
    /** A flag set if the executor can be managed. 线程执行器是否需要管理的标志位 */
    private boolean manageableExecutor;
    
    /** The default pool size. 默认的线程池大小：16 */
    private static final int DEFAULT_MAX_POOL_SIZE = 16;

    /** The number of thread to create at startup. 启动时需创建的线程数 */
    private static final int BASE_THREAD_NUMBER = 0;

    /** The default KeepAlive time, in seconds. 默认的KeepAlive事件：30秒 */
     ...
     * 
     * 一个标志位，用来标记Executor是自己创建的还是外部作为参数传进来的。
     */
     ...
     * 
     * 构造方法：创建一个OrderedThreadPoolExecutor实例，初始化线程数0，最大线程数16。
     */
     ...
     * by this default executor.
     * 
     * 构造方法：创建一个OrderedThreadPoolExecutor实例，初始化线程数0，最大线程数为maximumPoolSize。
     * 
     ...
     * by this default executor.
     * 
     * 构造方法：创建一个OrderedThreadPoolExecutor实例，初始化线程数corePoolSize，最大线程数为maximumPoolSize。
     *
     ...
     * eventTypes field.
     * 
     * 使用指定eventTypes创建EnumSet，如果eventTypes为空就是默认值。
     *
